...
inline void CountMapToVecs(const map<uint, unsigned> &Map,
  vector<uint> &Keys, vector<unsigned> &Counts)
	{
	Keys.clear();
	Counts.clear();
	vector<uint> Keys1;
	vector<unsigned> Counts1;
	for (map<uint, unsigned>::const_iterator p = Map.begin(); p != Map.end(); ++p)
		{
		Keys1.push_back(p->first);
		Counts1.push_back(p->second);
		}
