...
// Global terminal gaps
	...
	static const AlnParams *GetGlobalAP();
	...
	float ScoreLocalPathMasked(const byte *A, const byte *B, const char *Path) const;
	float ScoreLocalPathIgnoreMask(const byte *A, const byte *B, const char *Path) const;
	static const float *const *GetSubstMx();
