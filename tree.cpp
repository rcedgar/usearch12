...
void cmd_tree_subset()
	{
	const string &InputFileName = opt(tree_subset);
	...
	vector<unsigned> LeafNodeIndexes;
	const unsigned LabelCount = SIZE(Labels);
	vector<string> NotLeaves;
	for (unsigned i = 0; i < LabelCount; ++i)
		{
		const string &Label = Labels[i];
		unsigned NodeIndex = T.GetNode(Label);
		if (!T.IsLeaf(NodeIndex))
			{
			NotLeaves.push_back(Label);
			continue;
			}
		LeafNodeIndexes.push_back(NodeIndex);
		}

	if (!NotLeaves.empty())
		Warning("%u labels are not leaves", SIZE(NotLeaves));

	Tree Subtree;
	T.MakeSubset(LeafNodeIndexes, Subtree);

	if (optset_newickout)
		Subtree.ToNewickFile(opt(newickout));
	if (optset_tabbedout)
		Subtree.ToTabbedFile(opt(tabbedout));
	}

void cmd_subtree()
	{
	const string &InputFileName = opt(subtree);
	if (!optset_node)
		Die("-node required");
	const unsigned NodeIndex = opt(node);
